 ...
 */
public class NewsAddLeadEditorActivity extends BaseActivity implements OnClickListener{
...
protected void onCreate(Bundle savedInstanceState) {
	...
	setContentView(R.layout.add_newlead_editor_layout);
...
private void initView() {
	button_edit_step=(Button)findViewById(R.id.add_button_edit_step);
	button_edit_save=(Button)findViewById(R.id.add_button_edit_save);
	editText_edit_jzxm=(EditText)findViewById(R.id.add_editText_edit_jzxm);
	editText_edit_jz_phone=(EditText)findViewById(R.id.add_editText_edit_jz_phone);
	linearLayout_edit_zt=(LinearLayout)findViewById(R.id.add_linearLayout_edit_zt);
	linearLayout_edit_contactdate=(LinearLayout)findViewById(R.id.add_linearLayout_edit_contactdate);
	linearLayout_edit_source=(LinearLayout)findViewById(R.id.add_linearLayout_edit_source);
	textView_edit_contactdate=(TextView)findViewById(R.id.add_textView_edit_contactdate);
	textView_edit_source=(TextView)findViewById(R.id.add_textView_edit_source);
	textView_edit_zt=(TextView)findViewById(R.id.add_textView_edit_zt);
...
public void onClick(View view) {
	switch (view.getId()) {
	case R.id.add_button_edit_step:
		//上一步
		finish();
		break;
	case R.id.add_button_edit_save:
		//保存
		getUpdateNetCustomerMsg();
		break;
	case R.id.add_linearLayout_edit_zt:
		Intent intent2=new Intent(this,NewLeadStateActivity.class);
		intent2.putExtra("resultCode", "5151");
		startActivityForResult(intent2, 5252);
		break;
	case R.id.add_linearLayout_edit_contactdate:
		popupWindow();
		break;
    case R.id.add_linearLayout_edit_source:
		Intent intent3=new Intent(this,NewLeadSourceActivity.class);
		intent3.putExtra("resultCode", "6161");
		startActivityForResult(intent3, 6262);
		break;
...
private void getUpdateNetCustomerMsg() {
	...
	if(validatePhoneAndPwd()){	
		...
		params.addBodyParameter("id",Customer_id);
		params.addBodyParameter("o","json");
					
		new NetUtil(this).sendPostToServer(params, Constants.API_CUSTOMER_UPDATE_FORSUB, NewsAddLeadEditorActivity.this,"updateCustomer");
...
public void onCompleted(String result, boolean isSuccess, String api,String action) {
	...
	if(api.equals(Constants.API_CUSTOMER_UPDATE_FORSUB)&&action.equals("updateCustomer")){
		if(status.equals(Status.STATUS_SUCCESS)){//请求成功 		
			...
			intent.putExtra("customerr_Id", Customer_id);
